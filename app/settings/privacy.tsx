...
const PRIVACY_SECTIONS = [
  ...
  { title: 'Googleユーザーデータの取り扱い', body: '本アプリはGmail連携機能においてGoogle APIを使用します。\n\n【アクセスするGoogleユーザーデータ】\n・Googleアカウントのメールアドレス（送信元アドレスの表示に使用）\n・Gmail送信権限（ユーザーが作成したメールの送信に使用）\n\n【Googleユーザーデータの共有・転送・開示先】\n・Googleユーザーデータ（アクセストークン・メールアドレス等）は、いかなる第三者にも共有・販売・転送・開示しません\n・アクセストークンは端末内のセキュアストレージにのみ保存され、外部サーバーに送信されません\n・メール送信処理はGoogle Gmail API（Google LLC）に対してのみ行われます\n\n【利用制限】\n・Googleユーザーデータは、ユーザーが要求したメール送信機能の提供にのみ使用します\n・広告目的やAI/MLモデルの学習・改善には使用しません\n\n本アプリのGoogle APIの利用はGoogle API Services User Data Policy（Limited Use要件を含む）に準拠しています。' },
